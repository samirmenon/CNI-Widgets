...
void setup()   {                
  ...
  Serial.println(micros()-startMs);
  for(byte b=0; b<=127; b+=10){
    oled.ssd1306_set_brightness(b);
    delay(100);
  }
  ...
  // draw a few filled rects 
  Serial.print("Fillrect: ");
  startMs = micros();
  for(byte x=0; x<100; x+=20)
    for(byte y=0; y<60; y+=10)
      oled.fillrect(x, y, 9, 5, WHITE);
  oled.display();
  Serial.println(micros()-startMs);
  delay(2000);
  Serial.print("Fillrectsmall: ");
  startMs = micros();
  for(byte x=0; x<100; x+=20)
    for(byte y=0; y<60; y+=10)
      oled.fillrectsmall(x+10, y, 9, 5, WHITE);
  oled.display();
  Serial.println(micros()-startMs);
  delay(2000);
  oled.clear();
 
  Serial.print("Fillrect black: ");
  startMs = micros();
  oled.fillrect(0, 0, 128, 64, WHITE);
  for(byte x=0; x<100; x+=20)
    for(byte y=0; y<60; y+=10)
      oled.fillrect(x, y, 9, 5, BLACK);
  oled.display();
  Serial.println(micros()-startMs);
  delay(2000);
  Serial.print("Fillrectsmall black: ");
  startMs = micros();
  for(byte x=0; x<100; x+=20)
    for(byte y=0; y<60; y+=10)
      oled.fillrectsmall(x+10, y, 9, 5, BLACK);
  oled.display();
  Serial.println(micros()-startMs);
  delay(5000);
  oled.clear();
